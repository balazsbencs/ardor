...
All op-amp audio outputs before their coupling capacitors have approximately 2.5 V DC bias. These nets are internal only. C401/C402/C502/C601/C602 now use Samsung CL31B106KBHNNNE (JLCPCB C89632): 10 µF ±10%, 50 V, nonpolar X7R ceramic in standard 1206 footprints. This increases the nominal coupling capacitance from 2.2 µF to 10 µF to lower the high-pass corner and reduce AC voltage across the ceramic. Effective capacitance depends on DC bias, temperature and aging; 50 V is the capacitor rating, not an external-input rating. Prototype bass distortion, microphonic response and startup/mute settling need checking. See `routing/SMD_CAPACITORS.md` for selection and verification details.
