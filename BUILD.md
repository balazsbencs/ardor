...
Every push to `main` runs `.github/workflows/release.yml`. When the push changes
image inputs, such as Buildroot configuration, device software, or the Manager
app embedded in the image, the workflow creates its own `v0.1.<run-number>` tag,
builds the compressed Raspberry Pi image, and publishes one GitHub Release after
the required checks pass. Documentation-only changes do not create a release or
build an image. Tags do not need to be created or pushed manually.
...
sources, and manager-daemon sources. Within a release run, the cache can reuse a
completed build when rerun. Each image contains its release version and commit,
so images are not reused across releases.
...
cat > /opt/ardor-pedal/wifi/wpa_supplicant.conf <<'EOF'
...
Replace `HU` with the device's operating country. The service uses `wlan0` and
obtains its address via DHCP. Confirm the connection with
`wpa_cli -i wlan0 status` and `ip addr show wlan0`.

For a trusted development image, the same configuration can be embedded during
the build. Store it outside Git (for example, in
`.secrets/wpa_supplicant.conf`), then run:

```sh
ARDOR_WIFI_CONFIG=.secrets/wpa_supplicant.conf ./scripts/build-image.sh
```

The configuration is mounted read-only into the build container and copied with
mode `0600` to the image's writable data partition. Anyone with physical access
to the SD card can recover this credential.
