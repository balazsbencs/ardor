...
export function AppShell({ onCloudDevices }: { onCloudDevices?: () => void } = {}) {
  ...
  return (
    <div className="app-shell" data-theme={theme} style={accentStyle}>
      <header className="app-topbar">
        ...
        <nav className="app-navigation" aria-label="App navigation"><button className={view === "workspace" ? "is-active" : ""} onClick={() => setView("workspace")}>Workspace</button>{!hostedCloud && <button className={view === "assets" ? "is-active" : ""} onClick={() => setView("assets")}><FolderOpen size={15} /> Assets</button>}</nav>
