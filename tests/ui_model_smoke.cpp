...
int main()
{
  ...
  auto tunerState = ardor::makeDemoUiState();
  ardor::enterTunerMode(tunerState);
  if (require(tunerState.mode == ardor::UiMode::Tuner, "tuner mode should be active")) return 1;
  ardor::updateTunerTelemetry(tunerState, {true, 82.4f, -2.1f, 0.95f, "E", 2});
  if (require(tunerState.tuner.signalDetected && tunerState.tuner.note == "E"
                && tunerState.tuner.octave == 2,
              "tuner telemetry should update")) return 1;
  ...
  for (const auto& asset : catalogState.assets) {
    ...
    if (asset.name == "Tape Delay") {
      ...
      if (require(asset.type == "delay" && asset.blockType == "delay" && asset.mode == "tape",
                  "Tape Delay should resolve to the working Daisy effect and delay category")) return 1;
    }
    if (asset.name == "Room Reverb") {
  ...
  if (require(ardor::parameterPageCount(catalogState) > 0,
              "demo Chorus should expose working controls")) return 1;
  ardor::selectBlock(catalogState, 2);
  if (require(ardor::parameterPageCount(catalogState) > 0,
              "demo Tape Delay should expose working controls")) return 1;

  auto revisionState = ardor::makeDemoUiState();
  const auto initialRevisions = revisionState.revisions;
  ardor::setUiStatus(revisionState, "Ready");
  if (require(revisionState.revisions.status == initialRevisions.status + 1
                && revisionState.revisions.chain == initialRevisions.chain,
              "status changes should invalidate only the status region")) return 1;
  auto revisionTelemetry = revisionState.telemetry;
  revisionTelemetry.callbacks = 1;
  ardor::updateRealtimeTelemetry(revisionState, revisionTelemetry);
  if (require(revisionState.revisions.telemetry == initialRevisions.telemetry,
              "non-visible telemetry should not invalidate LVGL")) return 1;
  revisionTelemetry.overBudget = 1;
  ardor::updateRealtimeTelemetry(revisionState, revisionTelemetry);
  if (require(revisionState.revisions.telemetry == initialRevisions.telemetry + 1,
              "visible telemetry should invalidate its retained label")) return 1;
  ardor::setMasterVolume(revisionState, 25);
  ...
  ardor::setCategoryFilter(state, "delay");
  if (require(state.categoryFilter == "delay", "delay category filter failed")) return 1;
  ardor::setCategoryFilter(state, "reverb");
  if (require(state.categoryFilter == "reverb", "reverb category filter failed")) return 1;
  ardor::setCategoryFilter(state, "utility");
  if (require(state.categoryFilter == "utility", "utility category filter failed")) return 1;
  ...
  if (require(splitState.bank.presets[splitState.activePreset].version == 2
                ...
                && splitState.bank.presets[splitState.activePreset].blocks[0].lanes[0].empty()
                && splitState.bank.presets[splitState.activePreset].blocks[0].lanes[1].empty(),
              "Split insertion should create a version-2 region with two empty lanes")) return 1;
  completePreview(splitState);
  ardor::openLaneBlockDrawer(splitState, 0, 1, 0);
  if (require(splitState.blockInsertRig == 0 && splitState.blockInsertLane == 1
                && splitState.blockInsertIndex == 0,
              "right-lane plus should retain its exact insertion target")) return 1;
  ardor::insertLaneAssetBlock(splitState, chorusAssetIndex, 0, 1, 0);
  if (require(splitState.bank.presets[splitState.activePreset].blocks[0].lanes[1].size() == 1
                && splitState.bank.presets[splitState.activePreset].blocks[0].lanes[1][0].type == "mod"
                && splitState.bank.presets[splitState.activePreset].blocks[0].lanes[1][0].params.value("mode", "") == "chorus",
              "lane insertion should append the selected effect to only the targeted rail")) return 1;
  completePreview(splitState);
  if (require(ardor::moveLaneBlock(splitState, 0, 1, 0, 0, 0),
              "lane effects should move between Left and Right")) return 1;
  if (require(splitState.bank.presets[splitState.activePreset].blocks[0].lanes[0].size() == 1
                && splitState.bank.presets[splitState.activePreset].blocks[0].lanes[1].empty()
                && splitState.bank.presets[splitState.activePreset].blocks[0].lanes[0][0].params.value("mode", "") == "chorus",
              "cross-lane moves should preserve effect data and lane order")) return 1;
  completePreview(splitState);
  ardor::selectLaneBlock(splitState, 0, 0, 0);
  ...
  if (require(!splitState.bank.presets[splitState.activePreset].blocks[0].lanes[0][0].enabled
  ...
  if (require(state.blockDrawerOpen && !state.paramDrawerOpen,
              "opening Blocks should close the parameter drawer")) return 1;
  ardor::selectBlock(state, 0);
  if (require(!state.blockDrawerOpen && state.paramDrawerOpen,
              "selecting a block should close Blocks and open parameters")) return 1;
  ...
  if (require(compressorControls.size() == 6 && compressorControls[0].label == "Threshold"
  ...
  const auto telemetryRevision = diskState.revisions.telemetry;
  ardor::updateClipDebugTelemetry(diskState, {true, true, "ir:cab", 0.5f, 10, 12});
  if (require(diskState.clipDebug.overloaded && diskState.clipDebug.firstStage == "ir:cab",
              "ui stores clip-debug summary")) return 1;
  if (require(diskState.revisions.telemetry == telemetryRevision + 1,
              "clip-debug changes invalidate telemetry")) return 1;
